...
- データソース:
  ...
  - YouTube: 10チャンネルをRSSで30分ごとに自動監視 + 過去の人気動画は毎日12:00に自動取得(手動ボタンでも即時実行可)
...
- 過去の人気動画は**毎日12:00に自動実行**(`Daily YouTube Backfill Trigger`)。「Run YouTube Backfill」ボタンで手動実行も可能。いずれも前回まだ報告していない新しい上位動画だけをレポート化するため、同じ内容が重複して報告されることはありません
...
### 過去の人気動画(毎日12:00に自動実行、手動ボタンでも実行可)

`Daily YouTube Backfill Trigger`(毎日12:00、Asia/Tokyo)で自動的に、または
